...
%                     coupling.
                      %%%%%%%这里假设对于每一条path，其所有20条subpath的XPD是相同的%%%%%%
%
%
...
% extract the number of links links的数目必须和LINKPAR中给出的各参数的长度相等
...
% check that LUT size is a power-of-two     %%%Look Up Table，用于ANSI-C中计算余弦函数查表%%%
%   
...
% 指，如果使用给定的pdp和AOS/AOD，那么path数就是固定的（6）。这里没有给定。生成方式应该是根据5章里的步骤生成的。还在下边。


% GENERATION OF RANDOM "BULK" PARAMETERS FOR ALL LINKS
switch (ni)    %%%ni 是什么？？是scm()的参数数目%%%
