...
首先，大规模MIMO系统模型需要建立恰当的信道模型，并且可以通过研究该信道的特点可以有针对的进行复杂度优化。所以本文从物理多径信道模型出发，分析了物理信道模型的参数含义，并建立物理信道模型与统计信道模型的关系。进而，提出了大规模MIMO波束域信道模型，分析了大规模MIMO波束域信道的特性，包括：DFT矩阵渐近解相关、波束与方向对应性以及波束域信道稀疏性。利用3GPP空间信道模型(SCM)，生成波束域信道样本，验证波束域信道的特性，为后续章节的研究打下了基础。

然后，检测算法的正确性需要使用合适的系统模型，本文根据信道模型加入发送接收部分建立完整的系统模型。并基于大规模MIMO无线传输的系统模型，阐述了上行传输中MMSE检测方法。分析MMSE检测数学表达式中复杂度最高的矩阵求逆运算，利用QR分解降低MMSE检测的计算复杂度。而利用QR分解的Givens变换在稀疏矩阵复杂度降低，以及波束域信道矩阵在经过DFT变换后信道矩阵稀疏的性质，提出了基于MMSE的低复杂度接收机。其将原始MMSE的计算复杂度$\Theta(M^3)$(M为基站接收天线数)化简至。将基于MMSE的低复杂度接收机性能与MMSE检测结果的作对比，得出此低复杂度接收机在降低复杂度的同时很好的保持了性能的结论。

最后，利用Cayley Hamilton理论得出了相较MMSE接收机拥有更低复杂度的PE接收机。针对MMSE检测中矩阵求逆复杂度高的问题，根据Cayley Hamilton理论，利用矩阵多项式逼近矩阵求逆结果。进而，在大规模MIMO系统中，利用算子自由概率推导了在矩阵维度较大时的确定性等同PE接收机，并分析了其复杂度，低复杂度的PE接收机具有本论文提出的几个低复杂度算法的最好的复杂度。最后利用仿真结果分析了低复杂度PE接收机的性能，以及分析其的不足。
\end{abstract}
...
At present, people's life is inseparable from mobile communication, and the development of the times is based on mobile communication to a certain extent. The popular emerging technologies such as Internet of Things, Intelligent Interconnection and Vehicle Networking need communication systems with high throughput, low latency, low power consumption and low hardware requirements. Therefore, the fifth generation mobile communication system (5th Generation) will be further promoted on the fourth generation mobile communication system to higher these indicators. Based on the background of Large-Scale MIMO technology, this paper mainly studies the signal detection methods in large-scale MIMO systems. Several low-complexity MMSE detection methods are obtained by simplifying the channel properties and detection mathematical expressions.



Firstly, based on the physical multipath channel model of MIMO, the mathematical expression of the received signal channel of each receiving antenna is obtained. The set is the physical model of MIMO channel. According to the multipath model adopted by MIMO channel, this paper also writes the statistical model of MIMO channel. The statistical model is the main system model used in actual research and simulation. Then from MIMO signal, this paper transits to large-scale MIMO channel, and proposes a large-scale MIMO beam-domain channel. Then, three characteristics of large-scale MIMO beam-domain channel are analyzed: DFT matrix decorrelation, corresponding beam and sparse characteristic matrix. The simulation model SCM model used in this paper is proposed. The specific parameters are set and the simulation results are given to verify the characteristics of the beam-domain channel. The channel model and channel properties are obtained, which lays a foundation for the following chapters.



Then, a Large-Scale MIMO system model is proposed as the basis of all the research and Simulation in this paper. Then this chapter puts forward the MMSE detection method, and expounds the mathematical principle and expression in detail. The channel simulation SCM model introduced is used to simulate the results of performance benchmark for subsequent chapters. Then, the matrix inversion calculation with the highest complexity in MMSE detection mathematical expression is analyzed, and the method of reducing complexity by using QR decomposition is expounded. The Givens transform based on QR decomposition reduces the complexity of the sparse matrix, and the sparse property of the channel matrix in the beam domain mentioned after DFT transformation. A low complexity receiver based on MMSE is proposed. By comparing the results of MMSE detection obtained in this chapter with those of low complexity, it is concluded that the low complexity receiver can reduce the complexity while maintaining the performance well.



Finally, we use Cayley Hamilton theory to get PE receiver with lower complexity than MMSE receiver. The system model and MMSE test results are used to analyze the performance of PE receiver. At this time, it can be found that the coefficients of PE receiver can utilize determinacy and other factors, while reducing complexity and maintaining performance. Then, based on the concrete deduction of operator free probability theory, it is proved that deterministic equivalence can really replace the original coefficients. Then, the deterministic equivalent PE receiver expression is given and its complexity is analyzed. It can be seen that the low complexity PE receiver has the best complexity of several low complexity algorithms proposed in this paper. Finally, the performance of low complexity PE receiver is analyzed by simulation results, and its shortcomings are analyzed.
\end{englishabstract}
...
\chapter{绪论}
\section{研究背景}
目前人们的生活中已经离不开移动通信，而时代发展一定程度上以移动通信为基础。最近30年，信息时代的技术飞速发展，移动通信技术也依托20世纪的理论基础和21世纪的硬件发展得到了长足的进步。从第一代移动通信技术(1G, 1th Generation)的模拟通信和频分多址(FDMA, Frequency Division Multiple Access)技术，到第二代移动通信技术(2G, 2th Generation)的数字通信和时分多址的(TDMA, Time Division Multiple Access)技术，到第三代移动通信技术(3G, 3th Generation)的蜂窝移动网络以及宽带码分多址(CDMA, Code Division Multiple Access)技术，到现在的第四代移动通信技术(4G, 4th Generation)的多天线技术和正交频分复用(OFDM, Orthogonal Frequecy Division Multiple)技术。目前流行的物联网，智能互联，车联网等新兴技术需要大吞吐，低时延，低功耗，硬件要求低的通信系统，所以第五代移动通信系统(5G, 5th Generation)将在第四代移动通信系统上进一步提高这些指标。
...
\section{MIMO信道统计模型}
以上是信道的物理模型，在处理信道时，为了方便处理，一般会使用物理模型特性推导的统计模型\cite{LixiaoS}，在使用统计模型时需要明确以下概念。
