...
%     当BsGainPattern是常数时，天线数目由scmpar里的系数决定。当是四维数组时，由这个数组的第一维决定   %
...
%     all element patterns are defined over the same azimuth grid. The same
%     applies for MsGainPattern and MsAnglesAz.
...
antpar=struct(  'BsGainPattern',{1},...                         % in general: [Number_of_antennas, 2, Elevation_points, Azimuth_points]
                ...
                'BSGainAnglesEl',{0},...                        % size [1 Elevation_points] (parameter ignored)   %%%该模型忽略了仰角，但是保留了这个参数%%%
                'BsElementPosition',[0.5],...                   % in wavelengths. When scalar, uniform spacing assumed,相对距离，这里是两发两收，所以是0.5。三根天线时是类似（0.5，1.0），类推
                'MsGainPattern',{1},...
